...
const validDraft = {
  text: "  整理会议记录  ",
  dueAt: "2026-09-08T18:30",
};

test("待办草稿校验会拒绝空内容、无效时间", () => {
  assert.equal(validateTodoDraft({ ...validDraft, text: "" }).valid, false);
  assert.equal(validateTodoDraft({ ...validDraft, dueAt: "2026-09-08" }).valid, false);
  assert.equal(validateTodoDraft({ ...validDraft, dueAt: "2026-02-31T10:00" }).valid, false);
...
test("创建待办保存文本、时间并初始化为未完成", () => {
  ...
  assert.equal(todo.dueAt, "2026-09-08T18:30");
...
test("过期且未完成的待办会被清理，已完成和未过期待办保留", () => {
  ...
  const overdue = createTodo(
    { text: "过期任务", dueAt: "2026-09-08T17:59" },
    new Date("2026-09-08T10:00:00.000Z"),
  );
  const notDue = createTodo(
    { text: "未来任务", dueAt: "2026-09-08T18:01" },
    new Date("2026-09-08T10:00:00.000Z"),
  );
  const completed = setTodoCompleted(
    createTodo(
      { text: "已完成过期任务", dueAt: "2026-09-08T12:00" },
      new Date("2026-09-08T10:00:00.000Z"),
    ),
    true,
    new Date("2026-09-08T11:00:00.000Z"),
